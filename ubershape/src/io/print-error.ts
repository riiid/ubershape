...
import { ReadResult } from "./read-schema.ts";
import { Span } from "../parser/recursive-descent-parser.ts";
import { Def, Select } from "../parser/ast.ts";
...
const ubershapeValidationErrorPrinters: UbershapeValidationErrorPrinters = {
  ...
  [UbershapeValidationErrorType.DefDuplicateType](
    error,
    println,
    printAroundText,
  ) {
  ...
  [UbershapeValidationErrorType.DefDuplicateEnumValue](
    error,
    println,
    printAroundText,
  ) {
  ...
  [UbershapeValidationErrorType.RecordDuplicateFieldName](
    error,
    println,
    printAroundText,
  ) {
...
const subshapeValidationErrorPrinters: SubshapeValidationErrorPrinters = {
  [SubshapeValidationErrorType.SelectReference](
    error,
    println,
    printAroundText,
  ) {
  ...
  [SubshapeValidationErrorType.FieldSelectorReference](
    error,
    println,
    printAroundText,
  ) {
  ...
  [SubshapeValidationErrorType.TypeSelectorReference](
    error,
    println,
    printAroundText,
  ) {
  ...
  [SubshapeValidationErrorType.ValueSelectorReference](
    error,
    println,
    printAroundText,
  ) {
...
function printDef(def: Def, printAroundText: PrintAroundTextFn) {
  switch (def.kind) {
    case "root":
      printAroundText({ start: def.start, end: def.start + 4 });
      break;
    case "union":
    case "enum":
    case "record":
      printAroundText(def.name);
      break;
  }
}
...
function printSelect(select: Select, printAroundText: PrintAroundTextFn) {
  switch (select.kind) {
    case "select-root":
      printAroundText({ start: select.start, end: select.start + 11 });
      break;
    case "select-union":
    case "select-enum":
    case "select-record":
      printAroundText(select.typeName);
      break;
  }
}
